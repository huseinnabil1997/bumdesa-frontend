...
export default function DashboardUnitKanpus() {
  ...
  return (
    <Stack>
      ...
      <Scrollbar>
        <Box sx={{ display: 'flex', overflowX: 'auto', pb: 1, pt: 1, pr: 1, minWidth: 900 }}>
          {!isLoading && (
...
function ProductItem({ data }) {
  ...
  const handleClick = () => {
    router.push(`/kanpus/data-bumdesa/detail?id=${id}`);
  };
  ...
  return (
    <Card
      elevation={3}
      sx={{ width: 250, borderRadius: 1.5, cursor: 'pointer' }}
      onClick={handleClick}
    >
      <Image
        alt={name}
